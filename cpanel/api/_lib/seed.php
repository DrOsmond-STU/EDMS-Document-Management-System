<?php
// Padanan PHP dari src/data/seed.ts — data demo awal (33 dokumen, 16 pengguna, dst).
require_once __DIR__ . '/constants.php';

function edms_days_from_today(int $days): string {
    $anchor = new DateTimeImmutable('2026-07-25T00:00:00Z');
    $d = $anchor->modify(($days >= 0 ? '+' : '') . $days . ' days');
    return $d->format('Y-m-d');
}

function edms_functions_seed(): array {
    return [
...
function edms_standards_seed(): array {
    return [
...
function edms_build_documents(): array {
    $codeMap = edms_document_type_code();
    $seq = [];
    $documents = [];
    foreach (edms_seed_inputs() as $i => $input) {
        $key = $codeMap[$input['type']] . '-' . strtoupper($input['functionId']);
        $seq[$key] = ($seq[$key] ?? 0) + 1;
        $code = $key . '-' . str_pad((string)$seq[$key], 3, '0', STR_PAD_LEFT);
        $effOffset = $input['effectiveOffset'] ?? null;
        $createdAt = edms_days_from_today(($effOffset ?? -60) - 20);
        $updatedAt = edms_days_from_today(($effOffset ?? -30) - 2);
        $isFinal = $input['status'] === 'released' || $input['status'] === 'obsolete';
        $documents[] = [
            'id' => 'doc-' . ($i + 1),
            'code' => $code,
            'title' => $input['title'],
            ...
            'version' => $isFinal ? '1.0' : '0.1',
            'revisionNumber' => $isFinal ? 1 : 0,
            'effectiveDate' => isset($input['effectiveOffset']) ? edms_days_from_today($input['effectiveOffset']) : null,
            'reviewDate' => isset($input['reviewOffset']) ? edms_days_from_today($input['reviewOffset']) : null,
            'expiryDate' => isset($input['expiryOffset']) ? edms_days_from_today($input['expiryOffset']) : null,
            'keywords' => $input['keywords'],
            'content' => 'Ringkasan konten untuk "' . $input['title'] . '". Dokumen ini mengacu pada standar ' . implode(', ', $input['standards']) . ' dan berlaku untuk fungsi terkait.',
            'owner' => $input['owner'],
            'relations' => [],
            'createdAt' => $createdAt,
            'updatedAt' => $updatedAt,
...
function edms_build_revisions(array $documents): array {
    $revisions = [];
    foreach ($documents as $doc) {
        ...
        if (in_array($doc['status'], ['released', 'obsolete', 'approval'], true)) {
            $revisions[] = [
                'id' => $doc['id'] . '-rev1',
                'documentId' => $doc['id'],
                'revisionNumber' => 1,
...
function edms_drafting_projects_seed(): array {
    return [
        ['id' => 'dp-1', 'requestNumber' => 'REQ-2026-014', 'documentTitle' => 'SOP Manajemen Perubahan Kontraktor', 'documentType' => 'SOP', 'functionId' => 'hsse', 'standards' => ['ISO45001', 'SMK3'], 'initialClassification' => 'internal', 'reason' => 'Belum ada SOP baku untuk kontraktor yang bekerja di area berisiko tinggi.', 'requester' => 'Agus Prasetyo', 'currentStage' => 'rapat', 'meetings' => [
            ['id' => 'm1', 'sessionNumber' => 1, 'date' => edms_days_from_today(-14), 'budget' => 1500000, 'hasPhotoEvidence' => true, 'hasMinutesEvidence' => true, 'attendees' => ['Agus Prasetyo', 'Novita Handayani', 'Bayu Kusuma'], 'signaturesCollected' => true],
            ['id' => 'm2', 'sessionNumber' => 2, 'date' => edms_days_from_today(-3), 'budget' => 800000, 'hasPhotoEvidence' => true, 'hasMinutesEvidence' => false, 'attendees' => ['Agus Prasetyo', 'Novita Handayani'], 'signaturesCollected' => false],
        ], 'createdAt' => edms_days_from_today(-25), 'updatedAt' => edms_days_from_today(-3)],
        ['id' => 'dp-2', 'requestNumber' => 'REQ-2026-015', 'documentTitle' => 'Kebijakan Pengelolaan Vendor Kritis', 'documentType' => 'Kebijakan', 'functionId' => 'procurement', 'standards' => ['ISO9001', 'ISO37001'], 'initialClassification' => 'restricted', 'reason' => 'Kebutuhan kebijakan formal untuk vendor dengan dampak operasional tinggi.', 'requester' => 'Citra Wulandari', 'currentStage' => 'daftar_hadir', 'meetings' => [
            ['id' => 'm3', 'sessionNumber' => 1, 'date' => edms_days_from_today(-20), 'budget' => 500000, 'hasPhotoEvidence' => true, 'hasMinutesEvidence' => true, 'attendees' => ['Citra Wulandari', 'Yusuf Hidayat'], 'signaturesCollected' => true],
        ], 'createdAt' => edms_days_from_today(-30), 'updatedAt' => edms_days_from_today(-1)],
        ['id' => 'dp-3', 'requestNumber' => 'REQ-2026-016', 'documentTitle' => 'SOP Klasifikasi Data Pelanggan', 'documentType' => 'SOP', 'functionId' => 'infosec', 'standards' => ['ISO27001'], 'initialClassification' => 'confidential', 'reason' => 'Diperlukan untuk mendukung kepatuhan perlindungan data pribadi.', 'requester' => 'Intan Permatasari', 'currentStage' => 'finalisasi', 'meetings' => [
            ['id' => 'm4', 'sessionNumber' => 1, 'date' => edms_days_from_today(-40), 'budget' => 0, 'hasPhotoEvidence' => true, 'hasMinutesEvidence' => true, 'attendees' => ['Intan Permatasari', 'Dewi Anggraini'], 'signaturesCollected' => true],
            ['id' => 'm5', 'sessionNumber' => 2, 'date' => edms_days_from_today(-18), 'budget' => 0, 'hasPhotoEvidence' => true, 'hasMinutesEvidence' => true, 'attendees' => ['Intan Permatasari', 'Dewi Anggraini', 'Wahyu Setiawan'], 'signaturesCollected' => true],
        ], 'finalizedContent' => 'Draf final menunggu pengesahan Ratifier.', 'createdAt' => edms_days_from_today(-45), 'updatedAt' => edms_days_from_today(-2)],
        ['id' => 'dp-4', 'requestNumber' => 'REQ-2026-009', 'documentTitle' => 'Prosedur Klasifikasi & Penanganan Informasi', 'documentType' => 'SOP', 'functionId' => 'infosec', 'standards' => ['ISO27001'], 'initialClassification' => 'confidential', 'reason' => 'Selaras dengan Kebijakan Keamanan Informasi yang telah rilis.', 'requester' => 'Intan Permatasari', 'currentStage' => 'pengesahan', 'meetings' => [
            ['id' => 'm6', 'sessionNumber' => 1, 'date' => edms_days_from_today(-60), 'budget' => 0, 'hasPhotoEvidence' => true, 'hasMinutesEvidence' => true, 'attendees' => ['Intan Permatasari'], 'signaturesCollected' => true],
        ], 'finalizedContent' => 'Menunggu pengesahan akhir oleh Ratifier.', 'ratifiedDocumentId' => 'doc-12', 'createdAt' => edms_days_from_today(-70), 'updatedAt' => edms_days_from_today(-5)],
        ['id' => 'dp-5', 'requestNumber' => 'REQ-2026-002', 'documentTitle' => 'SOP Audit Internal', 'documentType' => 'SOP', 'functionId' => 'qa', 'standards' => ['ISO9001', 'ISO27001'], 'initialClassification' => 'internal', 'reason' => 'Revisi SOP audit internal mengikuti pembaruan siklus sertifikasi.', 'requester' => 'Rangga Pradipta', 'currentStage' => 'undangan_rapat', 'meetings' => [], 'createdAt' => edms_days_from_today(-8), 'updatedAt' => edms_days_from_today(-1)],
        ['id' => 'dp-6', 'requestNumber' => 'REQ-2026-017', 'documentTitle' => 'Formulir Evaluasi Pelatihan', 'documentType' => 'Formulir', 'functionId' => 'hc', 'standards' => ['ISO9001'], 'initialClassification' => 'internal', 'reason' => 'Formulir evaluasi saat ini belum mencakup penilaian efektivitas jangka panjang.', 'requester' => 'Ratna Sari', 'currentStage' => 'permintaan', 'meetings' => [], 'createdAt' => edms_days_from_today(-1), 'updatedAt' => edms_days_from_today(-1)],
    ];
}

function edms_notifications_seed(): array {
    return [
        ['id' => 'n1', 'category' => 'approval_pending', 'message' => 'Prosedur Klasifikasi & Penanganan Informasi menunggu persetujuan Anda.', 'documentId' => 'doc-12', 'read' => false, 'createdAt' => edms_days_from_today(-1)],
        ['id' => 'n2', 'category' => 'expiring_soon', 'message' => 'SOP Rekonsiliasi Keuangan Bulanan akan direview dalam 8 hari.', 'documentId' => 'doc-22', 'read' => false, 'createdAt' => edms_days_from_today(-1)],
        ['id' => 'n3', 'category' => 'expiring_soon', 'message' => 'Kebijakan Akses Sistem — Level Direksi mendekati jadwal review.', 'documentId' => 'doc-13', 'read' => false, 'createdAt' => edms_days_from_today(-2)],
        ['id' => 'n4', 'category' => 'assigned', 'message' => 'Anda ditugaskan meninjau Business Continuity Plan — Data Center.', 'documentId' => 'doc-28', 'read' => false, 'createdAt' => edms_days_from_today(-3)],
        ['id' => 'n5', 'category' => 'shared', 'message' => 'SOP Backup & Restore Data telah dibagikan kepada fungsi IT.', 'documentId' => 'doc-27', 'read' => true, 'createdAt' => edms_days_from_today(-5)],
        ['id' => 'n6', 'category' => 'expiring_soon', 'message' => 'SOP Pemeliharaan Preventif Mesin akan direview dalam 3 hari.', 'documentId' => 'doc-31', 'read' => false, 'createdAt' => edms_days_from_today(-1)],
        ['id' => 'n7', 'category' => 'approval_pending', 'message' => 'Business Continuity Plan — Data Center menunggu persetujuan Anda.', 'documentId' => 'doc-28', 'read' => false, 'createdAt' => edms_days_from_today(-2)],
        ['id' => 'n8', 'category' => 'assigned', 'message' => 'Rapat pembahasan ke-2 SOP Manajemen Perubahan Kontraktor telah selesai — bukti notulen belum diunggah.', 'read' => false, 'createdAt' => edms_days_from_today(-3)],
    ];
}

function edms_build_audit_log(array $documents): array {
    $log = [];
    ...
    $i = 0;
    foreach ($documents as $doc) {
        if ($doc['status'] === 'released' || $doc['status'] === 'obsolete') {
            $log[] = [
                'id' => 'al-status-' . $i,
    ...
    $log[] = ['id' => 'al-login-1', 'actor' => 'Wahyu Setiawan', 'action' => 'login', 'entity' => 'Session', 'entityId' => 'u7', 'timestamp' => edms_days_from_today(0), 'detail' => 'Login berhasil'];
    $log[] = ['id' => 'al-login-2', 'actor' => 'Ratna Sari', 'action' => 'login', 'entity' => 'Session', 'entityId' => 'u8', 'timestamp' => edms_days_from_today(0), 'detail' => 'Login berhasil'];
    $log[] = ['id' => 'al-master-1', 'actor' => 'Wahyu Setiawan', 'action' => 'master_data_change', 'entity' => 'Standard', 'entityId' => 'ISO22301', 'timestamp' => edms_days_from_today(-10), 'detail' => 'Menambahkan standar ISO 22301 ke Master Data'];
    $log[] = ['id' => 'al-export-1', 'actor' => 'Ratna Sari', 'action' => 'export', 'entity' => 'Report', 'entityId' => 'rpt-1', 'timestamp' => edms_days_from_today(-4), 'detail' => 'Mengekspor laporan ringkasan dokumen ke CSV'];
...
function edms_initial_state(): array {
    ...
    return [
        ...
        'draftingProjects' => edms_drafting_projects_seed(),
        'notifications' => edms_notifications_seed(),
        'auditLog' => edms_build_audit_log($documents),
